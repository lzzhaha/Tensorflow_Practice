...
def read_data(filename, vocab, window=NUM_STEPS, step=NUM_STEPS//2):
    for text in open(filename):
        text = vocab_encode(text, vocab)
        for start in range(0, len(text) - window, step=step):
...
def create_rnn(seq, hidden_size=HIDDEN_SIZE):
    ...
    output, outstate = tf.nn.dynamic_rnn(cell, seq, length, in_state)
